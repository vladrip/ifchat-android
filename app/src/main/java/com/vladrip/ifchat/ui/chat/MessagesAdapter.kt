...
class MessagesAdapter(
        ...
        val holder: ViewHolder = when (viewType) {
            R.layout.message_user ->
                UserMessageViewHolder(
                    inflater.inflate(R.layout.message_user, parent, false),
                    onMessageClick
                )
            ...
            R.layout.message -> {
                ...
                MessageViewHolder(view, chatType)
            }
    ...
    class MessageViewHolder(item: View, private val chatType: ChatType) : ViewHolder(item) {
        private val username = item.findViewById<TextView>(R.id.message_username)
        private val content = item.findViewById<TextView>(R.id.message_content)
        private val sentAt = item.findViewById<TextView>(R.id.message_sent_at)
    ...
    class UserMessageViewHolder(item: View, onMessageClick: View.OnClickListener) :
        ViewHolder(item) {
        private val body = (itemView as ViewGroup).getChildAt(0)
        private val content = item.findViewById<TextView>(R.id.message_content)
        private val sentAt = item.findViewById<TextView>(R.id.message_sent_at)
        private val status = item.findViewById<ImageView>(R.id.message_status)
    ...
    class DateSeparatorViewHolder(item: View) : ViewHolder(item) {
        private val date = item.findViewById<TextView>(R.id.date_separator_text)
